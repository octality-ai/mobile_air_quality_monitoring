#!/usr/bin/env python3
"""
Interactive 4-Wire Pump Controller Demo
========================================

Real-time pump control with live RPM feedback display.

Controls:
- UP/DOWN arrows:  Increase/decrease pump speed by 5%
- LEFT/RIGHT:      Increase/decrease pump speed by 1%
- 0-9:            Set pump to 0%, 10%, 20%, ... 90%
- F:              Full speed (100%)
- S:              Stop pump (0%)
...
- GPIO12 (Pin 32) → Blue wire (PWM control - INVERTED LOGIC)
- GPIO23 (Pin 16) → Green wire (Tachometer)
- Pi GND → Black wire (common ground with 12V supply)
- 12V Supply → Red wire

IMPORTANT: This pump uses inverted PWM (LOW=full speed, HIGH=stopped).
The code handles this automatically with active_high=False.
"""
...
class PumpDisplay:
    """Real-time display manager for pump status."""
    ...
    def _update_display(self):
        """Update the terminal display with current pump status."""
        # Get current values
        speed_percent = self.pump.get_speed_percent()
        rpm = self.pump.get_rpm()
        pulse_count = self.pump.get_pulse_count()
        ...
        # Print display
        print("=" * 70)
        print("        4-WIRE PUMP CONTROLLER - REAL-TIME MONITOR")
        print("=" * 70)
        print()
...
def interactive_ramp(pump, display):
    ...
    try:
        ...
        # Perform ramp
        pump.ramp_speed(target_percent, duration_seconds=duration_seconds)
...
def main():
    """Main interactive pump control program."""
    global display_kb_settings

    print("=" * 70)
    print("4-Wire Pump Controller - Interactive Demo")
    print("=" * 70)
    print("\nInitializing...")
    ...
    try:
        ...
        with KeyboardController() as kb:
            ...
            display.set_message("System ready. Use arrow keys to control pump.", duration=3.0)
            ...
            while running:
                ...
                if key:
                    current_speed = pump.get_speed_percent()

                    # Arrow keys - adjust speed
                    if key == 'UP':
                        new_speed = min(100, current_speed + 5)
                        pump.set_speed_percent(new_speed)
                    ...
                    # Number keys - set specific speeds
                        ...
                        speed = int(key) * 10
                        pump.set_speed_percent(speed)
                        display.set_message(f"Speed set to {speed}%")

                    # F - Full speed
                    elif key.upper() == 'F':
                        pump.set_speed_percent(100)
                        display.set_message("Full speed (100%)")

                    # S - Stop
                    elif key.upper() == 'S':
                        pump.stop()
                        display.set_message("Pump stopped")

                    # R - Ramp mode
                    elif key.upper() == 'R':
                        interactive_ramp(pump, display)
        ...
        print("Shutting down pump controller...")
        pump.cleanup()
        print("Done!")
